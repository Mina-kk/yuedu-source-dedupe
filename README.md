...
- 修复 YCK 主站/备用站打开问题：任一站点加载失败时自动切换另一站点，无需手动操作
- 兼容站点改版导致的 HTTP 错误（404/500），加载失败后自动恢复
- YCK 新增官方地址发布页入口（yckceo.vip），站点更换地址时可在应用内查看最新入口
- 修复 Android 15+ 返回键直接退出应用的问题：YCK 内返回上一页，主界面返回需确认
- YCK 站点选择菜单支持主站 / 备用站 / 发布页三个入口

## 2.1.4 稳定性优化

- 真正流式 JSON 解析：按 Reader 增量读取，避免大文件整段进内存
- 网络下载可 1～5 并发，大文件解析串行化，降低峰值内存
- 每次重新解析会重建引擎并释放旧结果，减少“第二次去重闪退”
- 本地已导入书源可在后续网络去重时作为种子保留
- 重复/错误详情默认上限调整为 200（统计数量仍完整）
- YCK 页面资源策略放宽：默认允许静态资源，仅拦截已知广告域
- YCK 仅在主框架加载失败时提示错误，并加入防抖
- 并发默认值改为 2，更适合 3 万级书源场景
...
- 内置 YCK 页面入口（主站 / 备用站 / 发布页），可收集受支持的书源链接
- 支持 1～5 个并发下载任务，可中止并保留已完成结果

## 系统要求

- Android 5.0（API 21）或更高版本
- 构建目标：Android API 36（基于 SDK Platform 34 编译）
...
预编译 APK 将发布在 GitHub 仓库的 **Releases** 页面。

当前版本（2.3.10）APK 的 SHA-256：

```text
02b7820e698bd6f86db04cc91101542a609f3bff9f133d985ba831e6b2ad0f08
```
